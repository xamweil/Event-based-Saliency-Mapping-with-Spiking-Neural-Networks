...
def train_on_centre_edge_map(ets, data_dir, ignore_file, split_ratio=(0.7, 0.15, 0.15), epochs = 10,
    ...
    train_scenes, val_scenes, test_scenes = ets.split_dataset(data_dir, ignore_file, split_ratio)
    ets.save_scene_lists(train_scenes, val_scenes, test_scenes, os.path.join(model_dir, "POC_"))
    ...
    optimizer = torch.optim.Adam([
        ...
        {'params': model.sconv_lstm1.parameters(), 'lr': 1e-3},
        {'params': model.rleaky.parameters(), 'lr': 1e-3}
    ], lr=1e-3)
    ...
    for epoch in range(epochs):
        ...
        ets.save_training_history(train_history, "train_history_epoch{}".format(epoch), save_dir=os.path.join(model_dir, "training_logs"))
        ets.save_training_history(val_history, "val_history_epoch{}".format(epoch), save_dir=os.path.join(model_dir, "training_logs"))
...
if __name__ == "__main__":
    ...
    model_dir = r"C:\Users\maxeb\Documents\Radboud\Master\Master Thesis\SNNsalienceMaps\models\V0_POC_test"
    os.makedirs(model_dir, exist_ok=True)
    train_on_centre_edge_map(ets, data_dir, ignore_file, epochs=8, model_dir=model_dir)
