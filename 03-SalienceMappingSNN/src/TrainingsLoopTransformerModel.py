...
from TransformerModelTest import Net
import torch
from ProgressPlotting import ProgessPlotting
import utils

def train_on_centre_egde_map(model_dir, epochs, resume_training: bool, dataset_dir = r"D:\SpikeDataSet",  split_ratio = (0.7, 0.15, 0.15), plot_progress = True, model_name=None):
    model = Net('cuda')
    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                ...
                prediction = model(batch["input"].to(torch.float))
            ...
            if plot_progress:
                ptp.update_plot(prediction[0].squeeze(), batch["label"][0, :, :], stats["tp"], stats["fp"],
                                stats["loss"],
                                os.path.join(model_dir, "TrainingPlots"),
        ...
        torch.save(model.state_dict(), os.path.join(model_dir, "TransformerEdgeModel_epoch_{}.pth".format(epoch)))
...
if __name__ == "__main__":
    model_dir = os.path.join(r"C:\Users\maxeb\Documents\Radboud\Master\Master Thesis\SNNsalienceMaps\models", "TransformerModel_V0")
    os.makedirs(model_dir, exist_ok=True)
    train_on_centre_egde_map(model_dir=model_dir, resume_training=False,  epochs=20)